...
|  Мощный API для сложных запросов |  Проект в режиме поддержки (но стабилен) |
...
| **Читаемость** |  Высокая |  Низкая |  Очень высокая |  Высокая |
| **Сложность настройки** |  Нет |  Средняя |  Средняя |  Средняя/Высокая |
| **Интеграция со Spring Data** |  Встроенная |  Встроенная |  Отличная |  Отдельная |
